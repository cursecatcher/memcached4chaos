...
int Assoc::assoc_insert(uint32_t hash, hash_item *it) {
    ...
    assert(assoc_find(hash, item_get_key(it), it->nkey) == NULL);
    ...
    do {
        ...
        for (int ii = 0; ii < hash_bulk_move && this->expanding; ii++) {
            ...
            for (it = this->old_hashtble[this->expand_bucket]; it; it = next) {
                ...
                bucket = hash(item_get_key(it), it->nket) & hashmask(this->hashpower);
...
void Assoc::assoc_expand() {
    ...
    if (this->primary_hashtable) {
        ...
        if (pthread_attr_init(&attr) != 0 ||
            ...
            (ret = pthread_create(&tid, &attr, assoc_maintenance_thread, this) != 0) {
