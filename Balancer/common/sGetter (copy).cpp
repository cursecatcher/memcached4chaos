...
sGetter::sGetter(int argc, char** argv){
    ...
    PrintServerList();
    char logFileName[32];
    sprintf(logFileName, "logs/log-GETTER-%s", sClientInfo.name);
   ...
   while(1){
        ...
        if(checkMigration==0){
            ...
            if(bytecount<=0){ // error
                ...
                printf("%s < %s: %d [%d B] - %d us\n",sClientInfo.name, sClientInfo.extName, msg.seq, bytecount, delay);
            }
...
//connect to BALANCER
    ...
    for(unsigned int i=1; i<parametersNumber; i++){
        ...
        case balancerIp_par:
            ...
            break;
        case setter_par:
            strcpy(sClientInfo.extName, argv[++i]);  // set the name of SETTER device to monitor
            printf("#Debug: SETTER name is %s", sClientInfo.extName);
            getchar();
            break;
        case pkg_size_par:
