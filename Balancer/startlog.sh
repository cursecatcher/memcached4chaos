...
2)      clear; echo "How many servers do you want to launch?"
	...
	clear; echo "Enter Balancer IP address (default: 192.168.1.1)"
	read BALANCER_ADDR
	if [ "$BALANCER_ADDR" == "" ]; then BALANCER_ADDR=" 192.168.1.1"; fi
	COUNTER=0
	  ...
	  do
	    ...
	    ./Server/bin/Release/Server $PORT $BALANCER_ADDR > "logs/SERVER_"${PORT}"_log" &
	    sleep 0.5
	  done
;;
...
3)      clear; echo "How many Setters do you want to launch?"
	...
	do
	   ./Setter/bin/Release/Setter -b $BALANCER_ADDR -n ${SETTER_NAME}${COUNTER} -s 1024  -ui 1000 > "logs/SETTER_"${SETTER_NAME}${COUNTER}"_log" &
	    let COUNTER=COUNTER+1
	    sleep 0.5
	done
;;
4)      clear; echo "How many Getters do you want to launch?"
	...
	clear; echo "Enter Balancer service IP address (default: 192.168.1.1)"
	read BALANCER_ADDR
	if [ "$BALANCER_ADDR" == "" ]; then BALANCER_ADDR=" 192.168.1.1"; fi
	COUNTER=0
	...
	do
	   ./Getter/bin/Release/Getter -b $BALANCER_ADDR -n ${NAME}${COUNTER} -s 1024 -ui 1000 -m ${SETTER_NAME} > "logs/GETTER_"${NAME}${COUNTER}"_log" &
	    let COUNTER=COUNTER+1
	    sleep 0.5
	done
;;
